...
test("the original 37 songs retain their exact order, titles, URLs, albums, and singers", () => {
  ...
  assert.equal(elements["tag-result-count"].textContent, "显示 43 / 43 首");
...
test("an additional data-only song renders and participates in search, singer counts, and random play", () => {
  ...
  assert.equal(list.children.length, 44);
  assert.equal(row.querySelector(".song-title-text").textContent, "44　New <song> & test");
  ...
  for (const query of [extra.title, "Test Singer", "NEW ALBUM", "New <song>, +sekai"]) {
    ...
    assert.equal(elements["tag-result-count"].textContent, "显示 1 / 44 首");
  }
  input.value = "sekai";
  input.dispatch("input");
  ...
  assert.equal(elements["tag-result-count"].textContent, "显示 44 / 44 首");
...
test("Japanese singer tags filter correctly and counts use Japanese", () => {
  ...
  assert.match(elements["tag-result-count"].textContent, /43 曲中/);
